# Introduction
The repository will setup and confgure a ECS/EC2 environment and deploy the Otel ECS service. The Otel is configured to be accepting traces on the endpoint http://<<alb_url>>:9080/v1/trace using the jaeger-thrift-splunk exporter.

# Part ONE 
The first part of the repository is the ecs_otel modules. 

## Usage

* Update the file otel_collector.json
  * SPLUNK_ACCESS_TOKEN
  * SPLUNK_REALM


# Part TWO
Terraform-AWS-ECS (The second part of the repository is extracted from https://github.com/arminc/terraform-ecs with some modification to the LB port inside the alb module ).
